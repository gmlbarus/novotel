...
$(document).ready(function(){
	...
	$("#flex1").flexigrid
			(
			{
			url: 'isi/kontrol/data-kontak-pelanggan.php',
			dataType: 'json',
			colModel : [
				...
				{display: 'Username', name : 'username', width : 170, sortable : true, align: 'left'},
				{display: 'Nama Pelanggan', name : 'nama', width : 170, sortable : true, align: 'left'},	
				],
			buttons : [
    ...
    if (com=='Send')
        {
		   ...
		   if(confirm('Sending to ' + $('.trSelected',grid).length + ' customers?')){
			...
			$.ajax({
			   ...
			   data: "items="+itemlist+"&id_faq=<?php echo $ms;?>",
		...
		if (com=='Send To All')
        {
		   if(confirm('Sending to all customers?')){
            $.ajax({
			   type: "POST",
			   dataType: "json",
			   url: "isi/kontrol/send-promosi.php",
...
<a href="?page=send_promosi_guest&ms=<?php echo $ms?>">Lihat data kontak pengunjung</a>
