...
if (!$sortname) $sortname = 'id_guestbook';
...
$sql = "SELECT * FROM guestbook $where $sort $limit";
$result = runSQL($sql);

$total = countRec('id_guestbook','guestbook',$where);
...
while ($row = mysql_fetch_array($result)) {
	if ($rc) $json .= ",";
		$json .= "\n{";
		$json .= "id:'".$row[0]."',";
		$json .= "cell:['".$row[0]."','".$row[1]."','".$row[7]."','".$row[2]."','";
		$json .= $row[3]."','".$row[4]."','".$row[5]."'";
