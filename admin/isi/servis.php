...
*/
			...
			searchitems : [
				{display: 'Servis', name : 'servis', isdefault: true}
				],
			sortname: "id_servis",
			sortorder: "asc",
			usepager: true,
    ...
    if (com=='Delete')
        {
		   ...
		   if(confirm('Delete ' + $('.trSelected',grid).length + ' items?')){
			...
			$.ajax({
			   type: "POST",
			   dataType: "json",
			   url: "isi/kontrol/delete-servis.php",
			   data: "items="+itemlist,
	...
	else if (com=='Edit')
        {
				...
				var vdata=items[0].id.substr(3);
					...
					document.location='?page=edit_servis&ms='+vdata;
				return false;
        }
    else if (com=='Add')
        {
		...
		else if (com=='View')
        {
				...
				var vdata=items[0].id.substr(3);
					...
					document.location='?page=view_servis&ms='+vdata;
				return false;
        }           
} 
</script>
