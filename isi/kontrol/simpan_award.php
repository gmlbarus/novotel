...
// else
{
		...
		$pagi = mysql_real_escape_string( implode(",",$_POST['pagi']) );
		$siang = mysql_real_escape_string( implode(",",$_POST['siang']) );
		$malam = mysql_real_escape_string( implode(",",$_POST['malam']) );
		$dessert = mysql_real_escape_string( implode(",",$_POST['dessert']) );
		$komentar = mysql_real_escape_string( $_POST['descript'] );
		if(empty($name) or empty($room) or empty($phone) ){
			die('<script>document.location="../../?page=award"</script>');
		}
		
		$query_simpan=mysql_query("insert into award 
			values('$name',$room,$phone,'$pagi','$siang','$malam','$dessert','$komentar')");
		if($query_simpan){
